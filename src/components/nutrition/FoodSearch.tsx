  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor: '#0D0D0D' }}>
      ...
      >
        ...
        <button onClick={onClose} className="p-2 rounded-full bg-white/10 text-white" aria-label="Fermer">
          <X size={18} />
        </button>
      </div>
        ...
        {mealOptions.map((m) => (
          <button
            ...
            className={`flex-1 py-1.5 rounded-full text-xs font-semibold transition-colors ${
              meal === m.id ? 'bg-accent text-white' : 'bg-white/10 text-text-secondary'
            }`}
          >
            {m.label}
          </button>
        ...
        <div className="flex flex-col flex-1 overflow-y-auto">
          ...
          <div className="p-4 space-y-4">
            <div className="bg-card rounded-xl p-4">
              <label className="text-text-secondary text-sm font-medium block mb-2">Quantité (grammes)</label>
          ...
          <div className="px-4 py-3 flex-shrink-0">
            <div className="flex items-center gap-3 bg-white/10 rounded-xl px-3 py-2.5">
              ...
              <input
                ...
                style={{ fontSize: '16px' }}
                className="flex-1 bg-transparent text-white placeholder-text-secondary border-0 outline-none"
                autoFocus
              />
              {query.length > 0 && (
                <button
            ...
            {query.trim() === '' ? (
              ...
              <div className="flex flex-col items-center justify-center h-40 gap-2">
                ...
                <p className="text-text-secondary text-sm text-center">
                  Tapez le nom d'un aliment pour le rechercher
                </p>
              </div>
            ) : results.length === 0 ? (
              /* No results — just text, no button (avoid keyboard-closing tap) */
              <div className="flex flex-col items-center justify-center h-40 gap-2">
                <p className="text-white/50 text-sm text-center">
                  Aucun résultat pour «{query}»
                </p>
                <p className="text-text-secondary text-xs text-center">
                  Modifiez votre recherche ou utilisez le ✕ ci-dessus
                </p>
              </div>
            ) : (
              /* Results list */
